...
def replace_params(json_data, params):
    key = params[0]
    if key in json_data:
        message_template = json_data[key]
        for i in range(1, len(params)):
            if params[i].startswith("entity."):
                if params[i] in json_data:
                    params[i] = json_data[params[i]]
            # 替换模板中的占位符
            message_template = message_template.replace(f"%{i}$s", params[i])
        return message_template
    else:
        return "Key not found in JSON data."
    
with open("qrobot_sakura/plugins/sakura/death_lang_zh_cn.json","r",encoding="utf-8") as file:
    death_data = json.load(file)
        ...
        try:
            ...
            async for message in websocket:
                ...
                if message['eventType'] == "join":
                    ...
                    tmp = replace_params(death_data,message['message'])
                    await bot.send_group_msg(group_id=group_id, message=f"{tmp}")
